...
if (file.exists(db_env)) {
  lgr$debug("Loading database environment", path = config::get('db_env'))
...
lgr$info("Connected to database", database = db_info$dbname, host = db_info$host, port = db_info$port, user = db_info$user)

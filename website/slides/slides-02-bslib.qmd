...
TK: page sidebar app `_examples/01-bslib/03_app.R`

Start with

1. State selector
2. Plot: predominant degree
3. Plot: cost vs earnings
...
TK: global-vs-local sidebar app `_exercises/03_app.R

Start with `_examples/01-bslib/03_app.R` and add:

* Radio buttons: locale type
* Radio buttons: group by

1. Convert the app to a sidebar page
2. Use `layout_sidebar()` to make the sidebar local to cost vs. earnings plot
...
Start with `_exercises/03_solution_app.R` with plots removed. Add text output for public, non-profit, for-profit values.
...
## Splat/columns example

TK: `_examples/01-bslib/04_app.R`

`rmedicine: examples/app-layout-columns_end.R`

Note: start with `lapply()` set up, write out `value_box()` code.
Demo: `layout_columns()` vs `layout_column_wrap()`, `width` vs `col_widths`, `row_heights`
...
TK: `_exercises/05_app.R`

Start with `_exercises/04_app.R` with value boxes. Add cost/earnings plot + new location card (`01-bslib/99_app.R`)
...
TK: `_examples/01-bslib/05_app.R`

Simple app with several inputs

* state selector
* locale type
* slider input

Use `accordion()` and `accordion_panel()` to organize the inputs.
